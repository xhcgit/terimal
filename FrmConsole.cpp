    ...
    ui(new Ui::FrmConsole)
{
    ...
    setSavedFileName(QDateTime::currentDateTime().toString("yyyyMMddHHmmss") + ".txt");
}
...
void FrmConsole::setSavedFileName(QString fileName)
{
...
/**
 * @brief FrmConsole::appendAndSaveAsString 以ASCII码追加显示，并将结果保存到txt文件中
 * @param ba Hex数组
 * @param code 校验结果
 */
void FrmConsole::appendAndSaveAsString(QByteArray ba, bool success)
{
...
/**
 * @brief FrmConsole::appendAndSaveAsHex 以Hex格式追加显示到文本框中
 * @param ba Hex数组
 * @param code 校验结果
 */
void FrmConsole::appendAndSaveAsHex(QByteArray ba, bool success)
{
    if(!isLocked)
    {
        static int count = 0;
