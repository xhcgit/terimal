...
//        p.close();
    ...
    /**
     * @brief NmeaLonStringToDouble Nmea格式的经度转成小数
     * @param strLon Nmea协议经度
     * @return 经度
     */
    static double NmeaLonStringToDouble(QString strLon)
    {
        bool ok;
        QString strDegreeOfLon = strLon.mid(0, 3);
        QString strMinuteOfLon = strLon.mid(3, 7);

        double degreeOfLon = strDegreeOfLon.toInt(&ok);
        double minuteOfLon = strMinuteOfLon.toDouble(&ok);
        
        return degreeOfLon + minuteOfLon/60;
    }

    /**
     * @brief NmeaLatStringToDouble Nmea格式的纬度转成小数
     * @param Lat Nmea协议纬度
     * @return 纬度
     */
    static double NmeaLatStringToDouble(QString strLat)
    {      
        bool ok;
        QString strDegreeOfLat = strLat.mid(0, 2);
        QString strMinuteOfLat = strLat.mid(2, 7);

        double degreeOfLat = strDegreeOfLat.toInt(&ok);
        double minuteOfLat = strMinuteOfLat.toDouble(&ok);

        return degreeOfLat + minuteOfLat/60;
    }
};
