...
void DataProcess::run() Q_DECL_OVERRIDE
{
    ...
    while(1)    //死循环一直读
    {
        if(isGpsNmeaHead())
        {
            ...
            emit revAPacketGpsNmeaData(nmea, success);    //发射读取到一帧nmea的信号
            initHead();     //清除header，进行下一帧读取
        }
        else if(isBdNmeaHead())
        {
            QByteArray nmea;
            nmea.append(NMEA_BD_HEAD_1);
            nmea.append(NMEA_BD_HEAD_2);
            ...
            bool success = MyTool::CheckNmeaCode(nmea);    //nmea数据校验
            emit revAPacketBdNmeaData(nmea, success);    //发射读取到一帧nmea的信号
            initHead();
        }
...
char DataProcess::readOneByte()
{
    ...
    //读取1个字节
    SharedData::usedRawData.acquire(1);    //usedRawData和freeRawData是用于生产者和消费者的同步信号量
 ...
 * @return success
 */
bool DataProcess::isGpsNmeaHead()
{
    return header.at(0) == NMEA_SYNC_HEAD_1
            &&header.at(1) == NMEA_SYNC_HEAD_2;
}

bool DataProcess::isBdNmeaHead()
{
    return header.at(0) == NMEA_BD_HEAD_1
            &&header.at(1) == NMEA_BD_HEAD_2;
}
